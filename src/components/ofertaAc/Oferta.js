...
import { setOption, startgetFacultades } from "../../actions/facultad";
...
export const Oferta = () => {
  ...
  const handleChange = (e) => {
    dispatch(setOption({ id: e.target.value, modo }));
  };
  ...
  return (
    <div className="bg-light container">
      ...
      >
        ...
        {modo === "facultad"
          ? facultades.map((f) => (
              <option value={f.id} key={f.id} name="section">
                {f.nombre}
              </option>
            ))
          : carreras.map((c) => (
              <option value={c.id} key={c.id} name="section">
                {c.nombre}
              </option>
            ))}
      </select>
